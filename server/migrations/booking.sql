...
ALTER TABLE `booking` CHANGE `status` `status` ENUM('PENDING','AWAITING','ACCEPTED','REJECTED','CANCELLED','COMPLETED') CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL; 
...
ALTER TABLE `booking` ADD `assigned_to` BIGINT NULL DEFAULT NULL AFTER `status`; 

ALTER TABLE `booking` ADD FOREIGN KEY (`assigned_to`) REFERENCES `account`(`id`) ON DELETE RESTRICT ON UPDATE RESTRICT; 